...
export const cargarCierreCaja = async (req, res) => {
  try {
    ...
    // Obtener total de efectivo y adelantos con tarjeta
    ...
    // Query para cantidad de ventas con tarjeta
    const [cantidadTarjetaResults] = await db.query(
      `SELECT COUNT(Ventas.id) AS cantidad_tarjeta, SUM(Ventas.total) AS total_tarjeta
      FROM Ventas
      WHERE Ventas.id_sucursal = ? 
      AND Ventas.metodo_de_pago = 'tarjeta'
      AND DATE(Ventas.fecha_venta) = ?`,
    ...
    // Query para cantidad de ventas en efectivo
    const [cantidadEfectivoResults] = await db.query(
      `SELECT COUNT(Ventas.id) AS cantidad_efectivo, SUM(Ventas.total) AS total_efectivo
      FROM Ventas
      WHERE Ventas.id_sucursal = ? 
      AND Ventas.metodo_de_pago = 'efectivo'
      AND DATE(Ventas.fecha_venta) = ?`,
    ...
    if (fondoRecibido > 0 && parseFloat(entryMotivo2.sobrante) !== 0) {
      return res.status(380).json({ message: "El sobrante debe ser 0 si hay fondo disponible." });
    }

    // Si no hay fondo, permitir cualquier valor de sobrante
    if (fondoRecibido === 0 && parseFloat(entryMotivo2.sobrante) < 0) {
      return res.status(390).json({ message: "El sobrante no puede ser negativo si no hay fondo." });
    }

    // Generar un ID único para el cierre de caja
    const idCierreCaja = crypto.randomUUID();
    const fechaCierreCaja = new Date().toLocaleDateString("en-CA").replace(/-/g, "/");
      ...
      if (id_motivo === 2) {
        await db.query(
          ...
          [
            ...
            cantidadTarjetaResults[0].total_tarjeta,         // Total de ventas con tarjeta
            efectivoResults[0].total_efectivo,       // Total de ventas en efectivo
            totalMontos                  // Total de montos calculados
          ]
        );
      } else {
        await db.query(
